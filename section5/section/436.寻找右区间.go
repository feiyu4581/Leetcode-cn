...
func findRightInterval(intervals [][]int) []int {
	starts := make([]pair436, len(intervals))
	ends := make([]pair436, len(intervals))

	for i, interval := range intervals {
		starts[i] = pair436{value: interval[0], index: i}
		ends[i] = pair436{value: interval[1], index: i}
	}

	sort.Slice(starts, func(i, j int) bool {
		return starts[i].value < starts[j].value
	})

	sort.Slice(ends, func(i, j int) bool {
		return ends[i].value < ends[j].value
	})

	res := make([]int, len(intervals))
	left := 0
	for _, right := range ends {
		for left < len(intervals) && starts[left].value < right.value {
			left += 1
		}
		...
		if left >= len(intervals) {
			...
			res[right.index] = starts[left].index
		}
	}
