...
func superPow(a int, b []int) int {
	...
	res := 1
	current := a
	current2, current3, current5 := 0, 0, 0
	for i := len(b) - 1; i >= 0; i-- {
		current2 = multi(current, current)
		current3 = multi(current2, current)
		current5 = multi(current2, current3)

		switch b[i] {
		case 1:
			res = multi(res, current)
		case 2:
			res = multi(res, current2)
		case 3:
			res = multi(res, current3)
		case 4:
			res = multi(res, multi(current2, current2))
		case 5:
			res = multi(res, current5)
		case 6:
			res = multi(res, multi(current5, current))
		case 7:
			res = multi(res, multi(current5, current2))
		case 8:
			res = multi(res, multi(current5, current3))
		case 9:
			res = multi(res, multi(current2, multi(current5, current2)))
		}

		current = multi(current5, current5)
	}
...
func Test372() {
	...
	fmt.Println(superPow(2147483647, []int{2, 0, 0}) == 1198)
